...
export default function NewsForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = id
      ? await supabase.from("news").update(dataToSubmit).eq("id", id)
      : await supabase.from("news").insert([dataToSubmit]);
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
        ...
        <div className="border border-accent-light rounded-lg p-4 space-y-3 bg-accent-light/20">
          ...
          <textarea
            placeholder="e.g., Buatkan rangkuman berita singkat tentang teknologi terbaru..."
            value={aiPrompt}
